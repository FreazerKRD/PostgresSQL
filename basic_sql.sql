...
FROM people AS p
    JOIN education AS e ON p.id = e.person_id
    JOIN sample AS s on p.company_id = s.id
GROUP BY p.id;
...
SELECT _1.name AS acquiring_company_name,
    a.price_amount AS acquisition_price,
    _2.name AS acquired_company_name,
    _2.funding_total,
    ROUND(a.price_amount / _2.funding_total) AS percentage
FROM acquisition AS a
JOIN company AS _1 ON a.acquiring_company_id = _1.id
JOIN company AS _2 ON a.acquired_company_id = _2.id
WHERE a.price_amount != 0
    AND _2.funding_total != 0
...
WITH invest AS
    (SELECT EXTRACT(MONTH FROM fr.funded_at) AS month_number,
        COUNT(DISTINCT f.name) AS company_number
...
acq AS    
    (SELECT EXTRACT(MONTH FROM a.acquired_at) AS month_number,
        COUNT(*) AS acquired_company_number,
        SUM(price_amount) AS total_acquisition
    FROM acquisition AS a
    WHERE acquired_at BETWEEN '2010-01-01' AND '2013-12-31'
    GROUP BY month_number)
SELECT i.month_number,
    i.company_number,
    a.acquired_company_number,
    a.total_acquisition
FROM invest AS i
    JOIN acq AS a ON i.month_number = a.month_number;
